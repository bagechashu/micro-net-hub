...
// List 获取数据列表
func (s UserService) List(req *UserListReq) ([]*User, error) {
	var list []*User
...
func (s UserService) ListCount(req *UserListReq) (int64, error) {
	var count int64
	db := global.DB.Model(&User{}).Order("id DESC")
...
// List 获取数据列表
func (s UserService) ListAll() (list []*User, err error) {
	err = global.DB.Model(&User{}).Order("created_at DESC").Find(&list).Error
...
func (s UserService) Exist(filter map[string]interface{}) bool {
	var dataObj User
	err := global.DB.Where(filter).First(&dataObj).Error
	return !errors.Is(err, gorm.ErrRecordNotFound)
}

// Find 获取同名用户已入库的序号最大的用户信息
func (s UserService) FindTheSameUserName(username string, data *User) error {
	return global.DB.Where("username REGEXP ? ", fmt.Sprintf("^%s[0-9]{0,3}$", username)).Order("username desc").First(&data).Error
}

// Find 获取单个资源
func (s UserService) Find(filter map[string]interface{}, data *User) error {
	return global.DB.Where(filter).Preload("Roles").First(&data).Error
}
...
func (s UserService) Update(user *User) error {
	...
	if err == nil {
		userDb := &User{}
		global.DB.Where("username = ?", user.Username).Preload("Roles").First(&userDb)
...
func (s UserService) GetCurrentUserMinRoleSort(c *gin.Context) (uint, User, error) {
	// 获取当前用户
...
func (s UserService) Delete(ids []uint) error {
	...
	err := global.DB.Debug().Select("Roles").Unscoped().Delete(&users).Error
	if err != nil {
		return err
	} else {
		// 删除用户成功，则删除用户信息缓存
...
func (s UserService) GetUserByIds(ids []uint) ([]User, error) {
	// 根据用户ID获取用户信息
	var userList []User
	err := global.DB.Where("id IN (?)", ids).Preload("Roles").Find(&userList).Error
	return userList, err
...
func (s UserService) ChangePwd(username string, hashNewPasswd string) error {
	err := global.DB.Model(&User{}).Where("username = ?", username).Update("password", hashNewPasswd).Error
	...
	if err == nil {
		if found {
			...
			// 没有缓存就获取用户信息缓存
			var user User
			global.DB.Where("username = ?", username).Preload("Roles").First(&user)
			userInfoCache.Set(username, user, cache.DefaultExpiration)
		}
	}
...
func (s UserService) ChangeStatus(id, status int) error {
	return global.DB.Model(&User{}).Where("id = ?", id).Update("status", status).Error
}

// ChangeSyncState 更新用户的同步状态
func (s UserService) ChangeSyncState(id, status int) error {
	return global.DB.Model(&User{}).Where("id = ?", id).Update("sync_state", status).Error
}

// GetCurrentLoginUser 获取当前登录用户信息
// 需要缓存，减少数据库访问
func (s UserService) GetCurrentLoginUser(c *gin.Context) (User, error) {
	var newUser User

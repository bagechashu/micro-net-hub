...
// List 获取数据列表
func (s ApiService) List(req *ApiListReq) ([]*Api, error) {
	var list []*Api
...
// List 获取数据列表
func (s ApiService) ListAll() (list []*Api, err error) {
...
// Add 添加资源
func (s ApiService) Add(api *Api) error {
	return global.DB.Create(api).Error
}

// Update 更新资源
func (s ApiService) Update(api *Api) error {
	// 根据id获取接口信息
	var oldApi Api
...
// Find 获取单个资源
func (s ApiService) Find(filter map[string]interface{}, data *Api) error {
...
func (s ApiService) GetApisById(apiIds []uint) ([]*Api, error) {
	var apis []*Api
